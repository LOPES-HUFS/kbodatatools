...
def get_game_info_table(year,month):
    ''' 
        Args: year,month
        year: 원하는 게임의 연도
        month 원하는 게임의 월
    ...
    url = "https://www.koreabaseball.com/Schedule/Schedule.aspx?seriesId=0,9" # 시범=1 정규=0,9 포스트=3,4,5,7
...
def get_game_list(data):
    ...
    date=["2019"+i.split(".")[0]+"".join(re.findall("[0-9]",i.split(".")[1])) for i in data['날짜']]
